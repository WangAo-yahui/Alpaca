...
Write `market_assessment.summary` in Chinese. Treat `previous_portfolio` as the
most recent valid strategic plan, which may come from an earlier trading day.
Compare it with current broker facts, explain material thesis/allocation changes,
and preserve still-valid monitoring or accumulation intent. Never reuse its
prices, order states, validity window or account facts as current evidence.

...
- Write `market_summary` in Chinese; do not translate schema enums, symbols,
  source IDs or URLs.
- Attempt live web research before finalizing. If the web tool is available,
  `network_research.status=not_requested` is forbidden; local-only is allowed
  only after an actual tool-unavailable or tool-error outcome.

...
Read `data/input.json` and produce exactly 60 unique research candidates plus a
...
Python provides separate `python_shortlists.stock` and
`python_shortlists.etf` research-priority lists. Stocks and ETFs are never
...
- When web research succeeds, select 3–5 of the 60 candidates from the eligible
...
ETFs that are not already among the 60 selections and may deserve future

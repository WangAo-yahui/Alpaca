...
- 每个 decision 的 `portfolio_action` 必须逐字复制对应上游 portfolio decision
  的 `action`，即使最新快照已把 `open` 标的显示为现有持仓也不得改成 `hold`；
  当前不应继续交易时只能用 `execution_decision=no_action/defer/reject` 和原因表达。

...
3. 对每个组合标的选择 approve、modify、defer、reject 或 no_action。
   输出的 `portfolio_action` 必须逐字等于该标的上游 portfolio `action`。
   最新快照中已经持有原计划 `open` 的标的时，仍保留 `portfolio_action="open"`；
   用 `execution_decision=no_action/defer/reject` 表达不重复建仓，不得擅自改为 `hold`。
